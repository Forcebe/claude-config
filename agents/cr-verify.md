...
2. **Writes go to localhost only.** `GET` any host the repo already talks to. `POST`/`PUT`/`PATCH`/`DELETE` only after confirming the base URL resolves to `localhost` or `127.0.0.1`, and only through the repo's own request wrapper.
