...
For HTML files: read [css-patterns.md](../visual-explainer/css-patterns.md) to match the existing page's styling when inserting the verification summary.

**Target file** — determine what to verify from $0:
...
- **HTML files**: insert a verification section as the final section before `</body>`, using the page's existing CSS custom properties and component patterns. Read the page's `<style>` block to identify its variable names and card classes — the summary must look native to the page, not bolted on. Use the page's card component (typically `.ve-card`) with a recessed or flat depth tier and a left-border accent: green if all claims confirmed, amber if corrections were made, red if significant errors were found. Use the page's status indicator pattern for per-claim results. If the page has section navigation (TOC), add a "Verification" entry to it.
...
**Phase 5: Report.** Tell the user what was checked, what was corrected, and re-open the file in the browser so they see the updated version (HTML) or report the path (markdown). If nothing needed correction, say so — the verification still has value as confirmation.
